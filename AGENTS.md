...
- **Restricted sudo**: `/etc/sudoers.d/openclaw` with `NOPASSWD` for wrapper scripts only: `safe-apt-install` (validates packages against allowlist of ~230 trusted packages), `safe-systemctl` (validates service name against allowlist of ~12 services + global actions like daemon-reload), `safe-pip-install` (validates pip packages against allowlist of ~130 trusted PyPI packages), `apt-get update`. Raw `sudo apt-get install`, `sudo pip3 install`, and `sudo systemctl` are NOT allowed — must go through wrappers
...
- **safe-systemctl**: `/usr/local/bin/safe-systemctl` — only allows actions (restart/start/stop/status/enable/disable/reload) on approved services (openclaw, tailscaled, ssh, fail2ban, auditd, docker, cron, etc.). Also allows global actions like `daemon-reload` without a service name
