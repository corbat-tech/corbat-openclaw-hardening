...
- **Systemd hardening for sudo**: `NoNewPrivileges=false`, `RestrictSUIDSGID=false`, `CapabilityBoundingSet=CAP_SETUID CAP_SETGID CAP_DAC_OVERRIDE CAP_FOWNER` — required for restricted sudo to work. All other hardening (ProtectSystem, ProtectKernel*, PrivateTmp, etc.) remains strict
