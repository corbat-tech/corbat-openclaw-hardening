...
- **Execution approvals**: `~/.openclaw/exec-approvals.json` — real schema uses `version: 1`, `socket` (path + auto-generated token), `defaults`, and `agents.main.allowlist` with `{ "pattern": "/usr/bin/..." }` entries (absolute paths, glob supported). 43 auto-approved patterns. Destructive commands (`rm`, `kill`, `systemctl`, `chmod`, `ssh`) require Telegram approval. `sudo`, `su`, `dd`, `reboot` never allowed. Approval requests forwarded via `approvals.exec` in `openclaw.json`
