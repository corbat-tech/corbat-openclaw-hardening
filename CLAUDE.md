...
- **SystemCallFilter**: Do NOT deny `@debug` — it causes core dumps with Telegram channel. Do NOT add `SystemCallFilter` in override.conf — causes NAMESPACE errors. `@privileged` removed from deny list to allow setuid()/setgid() for sudo
- **Systemd hardening (moderate)**: `ProtectSystem=full` (not strict — allows /var writes for apt), `NoNewPrivileges=false`, `RestrictSUIDSGID=false`, `CapabilityBoundingSet=CAP_SETUID CAP_SETGID CAP_DAC_OVERRIDE CAP_FOWNER`. `PrivateDevices`, `LockPersonality`, `RestrictRealtime` must be `false` (they implicitly force NoNewPrivileges=true). ReadWritePaths includes `/var/cache/apt`, `/var/lib/apt`, `/var/lib/dpkg`. Kernel protection maintained (ProtectKernel*, ProtectHostname, ProtectClock)
