...
- **Systemd hardening (moderate)**: Base service + override.conf pattern. Override resets `SystemCallFilter=` (empty), sets `ProtectSystem=full`, `PrivateDevices/LockPersonality/RestrictRealtime=false`. ReadWritePaths includes `/var/cache/apt`, `/var/lib/apt`, `/var/lib/dpkg`, `/var/log/apt`, `/tmp`. ProtectKernelTunables/Modules=false (apt needs them), ProtectKernelLogs=true, ProtectHostname/Clock=true. Override also contains API keys as `Environment=` lines
