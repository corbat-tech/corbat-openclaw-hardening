...
- **Systemd hardening (moderate)**: Base service + override.conf pattern. Override resets `SystemCallFilter=` (empty), sets `ProtectSystem=false`, `PrivateDevices/LockPersonality/RestrictRealtime/ProtectKernelTunables/ProtectKernelModules=false` — all 7 directives implicitly force `NoNewPrivileges=true`. With `ProtectSystem=false`, ReadWritePaths for /var/* and /tmp are unnecessary (removed). Only `/home/openclaw/*` ReadWritePaths remain (needed for `ProtectHome=read-only`). Override also contains API keys as `Environment=` lines (migrate to `EnvironmentFile=` recommended)
