...
- **Systemd hardening (moderate)**: Base service + override.conf pattern. Override resets `SystemCallFilter=` (empty), sets `ProtectSystem=false`, `PrivateDevices/LockPersonality/RestrictRealtime/ProtectKernelTunables/ProtectKernelModules=false` — all 7 directives implicitly force `NoNewPrivileges=true`. With `ProtectSystem=false`, ReadWritePaths for /var/* and /tmp are unnecessary (removed). `ProtectHome=false` — no ReadWritePaths needed. Only `PrivateTmp=true` remains (good practice, no side effects). API keys stored in `/etc/openclaw/env` (mode `600`, `root:openclaw`) loaded via `EnvironmentFile=` in the override — keys never appear in `systemctl show` output
