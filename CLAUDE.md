...
- **Systemd hardening (moderate)**: Base service + override.conf pattern. Override resets `SystemCallFilter=` (empty), sets `CapabilityBoundingSet=CAP_SETUID CAP_SETGID CAP_AUDIT_WRITE CAP_CHOWN CAP_DAC_OVERRIDE CAP_FOWNER`, `ProtectSystem=false`, `PrivateDevices/LockPersonality/RestrictRealtime/ProtectKernelTunables/ProtectKernelModules=false` — 8 directives that implicitly force `NoNewPrivileges=true` or block sudo. With `ProtectSystem=false`, ReadWritePaths for /var/* and /tmp are unnecessary (removed). `ProtectHome=false` — no ReadWritePaths needed. Only `PrivateTmp=true` remains (good practice, no side effects). API keys stored in `/etc/openclaw/env` (mode `600`, `root:openclaw`) loaded via `EnvironmentFile=` in the override — keys never appear in `systemctl show` output
