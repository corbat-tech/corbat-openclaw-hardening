...
- **Execution approvals**: `~/.openclaw/exec-approvals.json` with `security: "allowlist"`, `ask: "on-miss"`, `askFallback: "deny"`, `autoAllowSkills: true`. Approval requests forwarded to Telegram via `approvals.exec` in `openclaw.json`
